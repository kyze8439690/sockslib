...
public class Socks5DatagramSocket extends DatagramSocket {
    ...
    private static final String TAG = "Socks5DatagramSocket";
    ...
    public Socks5DatagramSocket(SocksProxy proxy) throws SocksException, IOException {
        ...
        Log.d(TAG, String.format("build datagram socket at[%s:%d]", this.getLocalAddress(),
                this.getLocalPort()));
        this.relayServerInetAddress = message.getIp();
        this.relayServerPort = message.getPort();

        Log.i(TAG, String.format("relay server's address[%s:%d]", relayServerInetAddress, relayServerPort));
    }
